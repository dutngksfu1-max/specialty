...
초등교사를 위한 **교직 성향·업무 스타일 탐색 플랫폼 "서치티쳐마인드"**.
...
- **MBTI 명칭, 4글자 유형 코드(예: 네 글자 조합), 기존 MBTI 유형명을 어디에도 노출하지 않습니다.**
- 적용 범위: UI 텍스트, URL/slug, `<title>`, metadata, OG 이미지, manifest, 공유 문구, 결과 이미지, 파일명, 주석
- 기존 MBTI 검사 문항을 복제하거나 문장만 바꿔 재사용하지 않습니다
- 로컬 폴더명이 `MBTI`이지만 **배포물에는 포함되지 않습니다**. 코드·문서에서 이 단어를 새로 만들지 마세요
...
결과 구성 순서(고정):
닉네임 → 결과 제목 → 한 줄 설명 → 나의 교직 리듬 → 4축 시각화 →
교실에서 빛나는 순간 → 바쁠 때 나타날 수 있는 모습 → 동료와 함께 일할 때 →
호흡이 자연스러운 스타일 → 조율하면 더 편한 스타일 → 이미지 저장 / 다시 검사하기
