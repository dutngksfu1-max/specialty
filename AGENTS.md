...
- 균형 자리가 있으면 그 자리의 두 글자를 모두 펼쳐 **후보를 나열합니다** (DEC-064).
  후보가 `maxCandidates`(4)를 넘으면 반쯤 잘라 보여 주지 않고 아예 나열하지 않습니다
...
강도 구간 (DEC-052)   |rawScore| 0 균형 / 1~6 조금 뚜렷 / 7~12 뚜렷 / 13~18 강함 / 19~24 매우 뚜렷
...
동점 보정 (DEC-063)   ① 장면평균  ② 극단응답  순서대로. 먼저 0이 아닌 값을 낸 쪽이 방향
                      ...
                      둘 다 0이면 → axis.defaultPole + directionSource "unresolved" = 진짜 균형
연속 점수             반드시 보존. 이산 결과 키만 남기지 않음. **보정은 rawScore를 바꾸지 않음**
```

**균형은 정확히 0점뿐입니다 (DEC-052). 이 폭을 넓히지 마세요.**

**균형 판정은 `directionSource === "unresolved"`로 합니다.** `isBalanced`(rawScore 0)나
강도 구간만 보면 동점 보정으로 방향을 찾은 축까지 균형으로 잡습니다 (DEC-063).
보정으로 방향을 찾은 축은 **가장 약한 방향 구간**으로 읽습니다.

축 점수는 정방향 6문항에서 역방향 6문항을 뺀 값이라 잡음이 0 근처에 모입니다(표준편차 약 4.9).
예전처럼 0~5를 균형으로 잡으면 **잡음 구간을 통째로 균형이라고 부르게 됩니다** —
무작위 응답에서 축의 73%, 네 축 모두가 28% 확률로 균형 판정을 받았습니다.
`balanceCalibration.test.ts`가 이 회귀를 막습니다.
...
| `axis-decision` | 판단할 때 먼저 살피는 것 | 원칙형 | 맥락형 |
...
결과 구성 순서(고정 — DEC-038로 확장, DEC-062로 두 갈래 분리):

**히어로(탭 밖, 항상 보임)**
닉네임 → 결과 제목 → 4렌즈 코드 → [유형별·성별별 캐릭터] → 나의 교직 리듬 → 밸런스 지도

**요약 보기(기본)** — 관점 4장(제목+막대) → 강점 1 · 신호 1 · 동료 1 → 함께할 때 1 · 맞출 점 1 → 내일 해 볼 것 1

**자세히 보기** — 관점 4장(제목+설명+막대+장면 예시) → 장면 차이 → 축 조합 해석(2장까지) →
이럴 때 강점이 돼요 → 여유가 줄었을 때 나타나는 모습 → 동료와 함께 일할 때 →
함께할 때 잘 이어지는 점 → 미리 맞춰 두면 좋은 점 → 내일 해 볼 것 → 동료와 나눌 질문

**마무리(탭 밖)** — 검사 안내 → 이미지 저장 / 다시 검사하기

같은 축 서술을 히어로와 본문에 나눠 싣지 않습니다. 축에 관한 것은 관점 카드 한 장이 모두 갖습니다 (DEC-062).

**결과에 넣지 않는 것**: 백분위·등급·순위 · 유형별 직업/업무 추천 · 하위척도
(신뢰도는 "많이 보여주기"가 아니라 "정직하게 보여주기"에서 나옵니다 — DEC-038)

**유형·성별 캐릭터 34종을 사용합니다 (DEC-050 · DEC-054).** 단 아래를 지킵니다.
...
- 균형 자리가 있으면 유형 캐릭터 대신 균형 전용 남녀 2종 중 선택값에 맞는 1종을 씁니다 (DEC-046)
- 전체 수는 `16유형 × 2성별 + 균형 × 2성별 = 34종`입니다

**4렌즈 코드는 결과 제목 다음 자리에 옵니다 (DEC-049).**
자리별 해설(`교류 G · 실제 A …`)을 함께 보여 주고, 균형 자리는 비우지 않고
**두 극의 글자를 함께 적습니다** (`R/C` — DEC-064). 자리 수는 그대로 넷입니다.
`ResultKey`(`pppp`)는 여전히 내부 식별자이며 화면에 노출하지 않습니다.

균형 축이 하나라도 있으면 내부 부호로 선택된 프로필의 제목·엠블럼·장면·협업·행동 문구를 화면에 쓰지 않고,
축별 균형 서술과 균형 전용 중립 안내를 사용합니다 (DEC-046).
제목과 교직 리듬은 `resultNarrative.balancedTitle` · `balancedRhythm`에서 가져옵니다 (DEC-062).
균형 축의 방향은 `axis.defaultPole`이 임의로 채우므로, 그 부호로 고른 제목을 쓰면 동전 던지기를 결과라고 부르게 됩니다.
