...
| `TypeCodeSpec.crosswalk` | 기본 접힘 UI의 문구 (검사 이름·안내·면책) |

- 네 글자 조립은 **화면에서만** 일어납니다. 소스에 네 글자 문자열이 존재하면 안 됩니다
- 환산 UI는 **기본 접힘**이고, 4렌즈 코드보다 작아야 합니다
...
닉네임 → 결과 제목 → 한 줄 설명 → [유형별·성별별 캐릭터] → 나의 교직 리듬 → 4축 시각화 →
축 조합 해석 → 강점이 드러날 수 있는 장면 → 바쁠 때 나타날 수 있는 모습 → 동료와 함께 일할 때 →
함께할 때 잘 이어지는 점 → 미리 맞춰 두면 좋은 점 →
