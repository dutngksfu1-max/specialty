...
export interface AxisNarrativeReading {
   ...
   *
   * ⚠️ **지금은 어느 화면에서도 쓰이지 않습니다** (DEC-074, 2026-09-06).
   *
   * 줄글 보기에 축을 하나씩 설명하는 카드가 있었고 이 원고가 거기 쓰였습니다.
   * 그 카드를 없앴습니다 — 네 방향을 따로 늘어놓으면 결과를 종합하지 않고
   * 코드 한 글자씩 해설하는 것이 되기 때문입니다. 지금은 그 내용이
   * `ResultProfile.portrait.inLessons` / `withStudents`에 장면 단위로 녹아 있습니다.
   *
   * 데이터는 남겨 둡니다. 나중에 축별 화면이 다시 필요할 수 있고, 지우면
   * 여덟 편을 다시 써야 하기 때문입니다. **이 필드를 살려 쓰기 전에 DEC-074를
   * 먼저 읽어 주세요.** 축을 하나씩 뜯어 보여 주는 화면은 되돌리지 않기로 했습니다.
   */
