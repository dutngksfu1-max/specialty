  ...
  it("반박하기 어려운 문형을 쓰지 않습니다", () => {
    for (const question of definition.questions) {
      ...
      */
      ...
      /*
        도덕적으로 기운 낱말 (2026-09-06 추가)

        한쪽 극이 나쁜 사람처럼 읽히면 그 극에 해당하는 교사도 자기 쪽을 못 고릅니다.
        `axis-decision`이 실제로 그랬습니다.

          원칙형(+) "사정이 있더라도 기준대로 **처벌**" · "노력보다 **성과**를 먼저"
          맥락형(-) "사정을 먼저 묻고 **참작**" · "그럴 사정이 있는지부터 듣는"

        문항 하나하나는 멀쩡한데 축 전체가 한쪽으로 기울어, 결과가 맥락형으로 쏠립니다.
        행동을 값매김 없이 적으면 이런 일이 생기지 않습니다.
      */
      expect(question.text, question.text).not.toMatch(
        /처벌|참작|포장하|직감에 의존|무시하|대충|게으|소홀/,
      );
    }
  });
