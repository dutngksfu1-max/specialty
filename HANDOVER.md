...
| 항목 | 내용 |
|---|---|
| 제품 | 초등교사용 **교직 성향·업무 스타일 탐색 플랫폼** "서치티쳐마인드" |
| 쓰이는 곳 | 교사 연수 강의 (아이스브레이킹 · 자기이해 · 동료 대화 소재) |
| MVP | 40문항(10×4 Part) · 5점 척도 · 4축 → 16개 결과 프로필 |
| 현재 상태 | **설계 문서 6종만 존재. 소스 코드 없음. `package.json`도 없음** |
| 다음 할 일 | Phase 1 — Next.js 초기화 + 채점 엔진 + 테스트 |
| 가장 큰 함정 | ① MBTI 관련 표현 노출 금지 ② 엔진에 콘텐츠 하드코딩 금지 |

**지금 당장 막히는 것은 없습니다.** Phase 1~3은 fixture(가짜 문항)로 진행 가능하며,
실제 콘텐츠가 필요한 시점은 Phase 4입니다.
...
동점(0)                isBalanced = true → 화면에 '균형' 표시 (DEC-001)
...
| DEC | 주제 | 결정 |
|---|---|---|
| 001 | 동점 처리 | '균형' 표시 + `defaultPole`로 프로필 확정 |
| 002 / 002b | 축 강도 | 3구간 라벨, 경계 0~4 / 5~12 / 13~20 |
| 003 | 브랜딩 | 서치티쳐마인드 (`searchteachermind`) |
| 006 | 저장 | idb |
...
| **D-2** (DEC-022) | 배포 주소는? | ① Vercel 기본 도메인(`*.vercel.app`)<br>② 별도 도메인 구매 | **①로 시작 → 실사용 시작하면 ②** (주소에도 MBTI 관련 표현 금지) |
...
| Phase | 내용 | 블로커 |
|---|---|---|
| 1 | 엔진 Foundation (도메인·채점·테스트) | 없음 |
| 2 | 랜딩 + 검사 진행 화면 (fixture로 완전 동작) | 그룹 B (5건) |
| 3 | 결과 화면 + 4축 시각화 + 이미지 저장 | 없음 |
| 4 | **실제 콘텐츠 주입** | 그룹 C (3건) ⭐ |
| 5 | PWA · 오프라인 · 접근성 QA | 그룹 D-1 |
| 6 | 오류 처리 · 성능 · 배포 | 그룹 D-2, D-3 |
...
| 결과에 "궁합" 표현 | 제품 톤 위반 | "호흡이 자연스러운 / 조율하면 더 편한" |
...
[프로젝트]
초등교사용 교직 성향·업무 스타일 탐색 플랫폼 "서치티쳐마인드"입니다.
...
[절대 규칙 — 위반 시 되돌려야 합니다]
1. MBTI 명칭·4글자 유형 코드를 어디에도 노출하지 않습니다.
   UI, URL/slug, <title>, metadata, OG 태그, manifest, 결과 이미지, 공유 문구 전부 포함입니다.
   로컬 폴더명이 MBTI이지만 배포물에는 포함되지 않습니다. 이 단어를 새로 만들지 마세요.
...
동점(0)   = isBalanced=true로 표시하되 결과는 하나로 확정. 연속 점수는 반드시 보존
...
먼저 문서를 읽고, 이해한 내용과 Phase 1 작업 계획을 요약해 주세요.
그 다음 제 승인을 받고 구현을 시작해 주세요.
```
