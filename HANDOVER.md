...
| MVP | 48문항(12×4 Part) · 5점 척도 · 4축 → 16개 결과 프로필 |
...
게이지 구간 (DEC-068)  |rawScore| 0~6 근소 / 7~12 분명 / 13~18 큰 차이 / 19~24 매우 큰 차이
방향                   >0 positive / <0 negative / =0 → 동점 보정 → axis.defaultPole
동점(0)                화면은 확정 방향을 설명하고, rawScore 0과 directionSource는 내부에 보존
...
| DEC | 주제 | 결정 |
|---|---|---|
| 063 / 068 | 동점·방향 처리 | 보정 후에도 같으면 기본 방향을 사용하고, 화면은 언제나 한 방향을 설명 |
| 068 | 게이지 구간 | 4구간 라벨, 경계 0~6 / 7~12 / 13~18 / 19~24 |
...
| **C-2** (DEC-024) | 문항 + 16개 결과 텍스트를 누가 쓸까요? | ① 전부 클라이언트<br>② 전부 개발자 초안 → 검수<br>③ 문항은 클라이언트, 결과 텍스트는 개발자 초안 | **③** — 이후 DEC-048로 문항을 48개로 확장했습니다 |
...
| 10 | **현재 콘텐츠** | `.../packages/teacher-style-v1/` | 48문항 + 16프로필 |
...
- `intensityBands` 0~6 / 7~12 / 13~18 / 19~24
- 각 축에 `defaultPole` 지정
...
범위      = 축당 12문항이면 -24 ~ +24
게이지 구간 = |rawScore| 0~6 근소 / 7~12 분명 / 13~18 큰 차이 / 19~24 매우 큰 차이
방향      = >0 positive, <0 negative, =0이면 동점 보정 후 axis.defaultPole 사용
동점(0)   = 내부 원점수는 0으로 보존하고 화면은 확정된 한 방향을 설명
...
[하지 말 것]
- 실제 문항이나 결과 텍스트를 임의로 창작하지 마세요. fixture만 씁니다.
...
| 실제 콘텐츠(48문항 + 16프로필) | ✅ DEC-023 / DEC-024 / DEC-048 완료 |
