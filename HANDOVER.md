...
> 작성일: 2026-08-19 (최종 갱신: **2026-09-06, 결과 화면 개편 DEC-069~074 완료 시점**)
> 받는 사람: 이 프로젝트를 이어서 개발할 개발자
>
> ⚠️ **이 문서의 4·5절은 Phase 0 시점 기준으로 쓰였습니다.**
> 현재 진행 상황은 아래 "현재 상태" 표와 `docs/decisions.md` 변경 이력을 보세요.
>
> ⚠️ **결과 원고를 만질 사람은 12절을 먼저 읽어 주세요.** 2026-09-06에 결과 화면과
> 문항을 크게 고쳤고, 그때 정한 규칙을 모르고 손대면 되돌리는 데 오래 걸립니다.
...
| 현재 상태 | **Phase 1~5 완료 + Phase 6 진행 중.** 콘텐츠 **6.0.0**, 결과 화면 세 갈래 |
| 다음 할 일 | C 교사 인지 인터뷰·익명 파일럿 → 근거 확보 후 B-2 최종 보정 |
| 배포 주소 | `https://specialty-nu.vercel.app` (DEC-022 확정) |
| 가장 큰 함정 | ① 기존 성격유형 검사 관련 표현 노출 금지 ② 엔진에 콘텐츠 하드코딩 금지 ③ **결과 원고 규칙 (12절)** |
...
**2026-09-06에 크게 바뀐 것** — 자세한 것은 12절과 `docs/decisions.md` DEC-069~074.

- 결과 화면에 세 번째 갈래 **「줄글 보기」**가 붙었습니다 (요약 보기·자세히 보기는 그대로)
- `axis-energy` 문항이 동료 장면 11개에서 **0개**가 되었습니다. 검사는 이제 **교실만** 잽니다
- 결과 원고를 전부 다시 썼습니다 — 유형당 성격 묘사 약 1,500자, 장면 8개
- 테스트 **329개**. 원고 품질(축 균형·문항 재진술·동료 표현·어투)이 검사로 들어왔습니다
...
└─ src/
   ├─ domain/                  채점·결과 계산 (프레임워크 import 금지)
   ├─ application/             유스케이스
   ├─ infrastructure/
   │  ├─ content/packages/teacher-style-v1/
   │  │  ├─ questions.ts       48문항 ← 장면 균형 규칙이 파일 머리에 있습니다
   │  │  ├─ profiles.ts        16유형 결과 원고 ← 가장 자주 고치는 파일
   │  │  └─ …
   │  └─ persistence/          IndexedDB
   ├─ features/result/         결과 화면 3종 (요약·자세히·줄글)
...
**결과 원고(`profiles.ts`)를 고칠 사람은 순서가 다릅니다.**

```
1. HANDOVER.md 12절            — 무엇이 왜 이렇게 되어 있는가
2. docs/decisions.md DEC-069~074 — 그 결정의 전말
3. AGENTS.md 「줄글 보기 원고를 쓸 때」 — 실제 작성 규칙 8가지
```
...
| — | 결과 화면 개편 (줄글 보기 · 문항 교실화 · 원고 전면 재작성) | ✅ 2026-09-06 완료 — DEC-069~074, **12절 참고** |
...
| 결과문에 동료·옆 반·교무실을 씀 | 이 검사는 교실을 잽니다 (DEC-074) | 선생님 자신·아이들·수업·업무 |
| 문항 문장을 결과에 옮겨 적음 | 답한 사람이 자기가 고른 문장을 다시 읽음 | 조합에서 추론 (12.4절) |
| 검사가 깨졌다고 문장부터 고침 | 낱말 목록이 불완전한 경우가 더 많았음 | 자를 먼저 확인 (12.5절) |
...
| 실제 콘텐츠(48문항 + 16프로필) | ✅ DEC-023 / DEC-024 / DEC-048 완료, DEC-071·074로 전면 개정 |
...
## 12. 결과 원고를 고치기 전에 (2026-09-06 · DEC-069~074)

> **이 절은 결과 화면과 `profiles.ts`를 만질 사람만 읽으면 됩니다.**
> 하루 동안 사용자 피드백을 받아 가며 고친 것을 정리한 것입니다.
> 모르고 손대면 되돌아가는 변경이 있습니다.

### 12.1 무엇이 문제였나

검사를 열심히 마친 교사에게서 **"결과가 와닿지 않는다"**는 피드백이 왔습니다.
파고들어 보니 원인이 여섯 갈래였습니다.

| 증상 | 원인 |
|---|---|
| 읽을 게 없다 | 히어로 요약이 119자, 세 문장에서 끝나기도 함 |
| 뭘 재는 검사인지 모르겠다 | 축 이름이 작고, 분류 이름(`몰입형`)·강도 이름(`근소한 차이`)이 시선을 뺏음 |
| 내 얘기 같지 않다 | 결과가 **문항을 그대로 옮겨 적고** 있었음 |
| 교직 검사인데 회식 얘기 | `axis-energy` 12문항 중 11개가 동료·혼자 장면 |
| 한 성향만 얘기한다 | 16유형 중 14개가 한 축으로 쏠려 있었음 (측정해 보고 알았습니다) |
| 무슨 말인지 두 번 읽어야 함 | 비유·추상어·문장성분 생략 |

### 12.2 그래서 지금 이렇게 되어 있습니다

**결과 화면은 세 갈래입니다.** 요약 보기 · 자세히 보기 · **줄글 보기**.
앞의 둘은 DEC-062 구조 그대로이고 **손대지 않기로 했습니다.**
새로 만든 줄글 보기(`ResultStoryView.tsx`)만 이번 규칙을 따릅니다.

```
카드 1 「선생님은 이런 교사입니다」
   여는 문단 → 아이들이 느끼는 선생님 → 무엇을 중요하게 여기는가
   → 선생님 교실은 이렇습니다 → 자주 듣는 오해

카드 2 「교실에서 드러나는 모습」
   수업을 만들 때 → 아이를 대할 때 → 강점이 되는 순간 → 여유가 줄었을 때
```

카드가 두 장뿐인 것은 의도입니다. 축을 하나씩 뜯어 보여 주던 카드,
장면 대비 카드, 동료 카드, 내일 할 일 카드를 **없앴고 그 내용은 위 구역에 녹였습니다.**

### 12.3 절대 하지 말 것

| 하지 말 것 | 왜 |
|---|---|
| 축을 하나씩 해설하는 카드를 되살리기 | 결과를 종합하지 않고 코드 한 글자씩 푸는 것이 됩니다 |
| 결과문에 동료·옆 반·교무실·연수 등장시키기 | 이 검사는 교실을 잽니다. 문항도 동료 장면 0/48입니다 |
| 문항 문장을 결과에 옮겨 적기 | 답한 사람이 자기가 고른 문장을 다시 읽게 됩니다 |
| 줄글 보기에 게이지·순위 배지·분류 이름 넣기 | 읽을 것 대신 외울 것을 줍니다 |
| 요약 보기·자세히 보기 건드리기 | 줄글 보기를 만들 때 한 약속입니다 |

`ResultProfile`의 `withColleagues` · `collaboration` · `nextSteps` · `talkingPoints`와
`AxisNarrativeReading.story`는 **일부러 남겨 둔 것**입니다. 요약·자세히 보기가 아직 씁니다.
지우지도 말고, 줄글 보기에 되살리지도 마세요.

### 12.4 원고를 쓸 때

전체 규칙은 `AGENTS.md`의 「줄글 보기 원고를 쓸 때」에 8개 항목으로 있습니다. 요약하면,

1. 장면(수업·아이·업무)으로 쓰고 축 이름은 화면에 내지 않습니다
2. 문항이 묻지 않은 것을 **조합에서 추론**합니다 — 여기서 공감이 나옵니다
3. 한 번 읽고 알아들어야 합니다. 비유·추상어·지시어 금지
4. `~합니다` 단정형, 주어는 `선생님` (`이 선생님` 아님)
5. `여유가 줄었을 때`만 `~수 있습니다`로 끝냅니다
6. 막는 것은 `성적이 오릅니다` 같은 **검사가 못 하는 약속**뿐입니다
7. 누가 그렇게 느끼는지를 비워 두지 않습니다 (`상대는` `밖에서는` 금지)
8. 장면 이름(`situation`)도 화면에 나옵니다. **반드시 `~ 때`로 끝나야 합니다**

> ⑧이 문법 규칙인 이유 — 장면은 화면에서 `<상황>에는 <설명>` **한 문장으로 조립**됩니다.
> 코드가 조사를 붙이므로 `~ 때`가 아니면 말이 깨집니다.
> 예전에 `생활지도할 때에서도`처럼 깨진 적이 있어 검사가 128개 전부를 봅니다.

### 12.5 원고 품질을 지키는 검사

```bash
pnpm vitest run   # 329개
```

| 파일 | 지키는 것 |
|---|---|
| `features/result/ResultStoryView.test.tsx` | 분류 이름·배지 금지 · 문항 재진술(어절 3-gram) · **축 균형** · 동료 표현 · 상대 비우기 · 장면 이름 문법 |
| `infrastructure/content/contentQuality.test.ts` | 어른 장면 비율 · 문항 중복 · 문체(오해·동기·감정) |
| `infrastructure/content/realContent.test.ts` | 축 간 어휘 누수 · 천장/바닥 문구 · 낙인 낱말 · 어미 |

**⚠️ 검사가 초록이라고 끝난 것이 아닙니다.**
2026-09-06에 검사가 전부 통과한 상태에서 사용자가 문제 문장을 직접 집어냈습니다.
낱말 목록에 그 표현이 없었기 때문입니다. 그래서 화면에 나오는 원고 전부
(리듬 16 · 성격 묘사 128문단 · 장면 128개)를 한 문장씩 읽어야 했고, 129곳을 고쳤습니다.

> **원고를 새로 쓰거나 크게 고쳤다면 검사를 믿지 말고 직접 읽어 주세요.**
> 반대로 **검사가 깨졌을 때는 문장을 고치기 전에 자를 먼저 의심하세요.**
> `되묻`은 있는데 `되물으면서`가 없고, `계획·즉흥` 목록에 정작 `계획`이 빠져 있어
> 멀쩡한 문장을 고칠 뻔한 일이 여러 번 있었습니다.

### 12.6 검사가 못 잡는 것 (사람이 봐야 합니다)

- 뜻이 겹치는 중복 — 낱말이 다르면 안 걸립니다
- 상대를 바꾸다 앞뒤가 깨진 문장
- 문항의 진짜 응답 쏠림 — 결국 응답 분포로만 확인됩니다 (B-2 보정 과제)
