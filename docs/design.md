...
| **친근함** | 문장이 사람 말투다. 딱딱한 전문용어를 쓰지 않는다 |
| **성숙함** | 교사를 어린이처럼 대하지 않는다. 캐릭터·이모지에 기대지 않는다 |
...
3. 카드는 정보를 묶는 도구이지 장식이 아니다. **카드 안에 카드를 넣지 않는다.**
4. 경계는 그림자보다 **선(border)**으로 만든다. 그림자는 떠 있는 요소에만 쓴다.
...
| 토큰 | 값 | 용도 |
|---|---|---|
| `radius-xs` | 4px | 배지, 작은 칩 |
| `radius-sm` | 6px | 입력 필드, 작은 버튼 |
| `radius-md` | 10px | **기본 버튼, 기본 카드** |
| `radius-lg` | 14px | 큰 카드(활성 검사 카드) |
| `radius-xl` | 18px | 결과 히어로 블록 |
| `radius-full` | 9999px | **라디오 원, 진행바 트랙에만** |

> **금지**: `radius-full`을 버튼·카드·입력에 쓰지 않습니다("모든 요소 pill" 금지).
> 카드 반경은 최대 18px입니다("지나치게 둥근 카드" 금지).
...
| 토큰 | 값 | 용도 |
|---|---|---|
| `elev-0` | `none` | **기본값.** 카드는 border로만 구분 |
| `elev-1` | `0 1px 2px oklch(0.30 0.02 90 / 0.06)` | 활성 검사 카드(살짝만) |
| `elev-2` | `0 2px 8px -2px oklch(0.30 0.02 90 / 0.08)` | 하단 고정 내비게이션 바 |
| `elev-3` | `0 8px 24px -8px oklch(0.30 0.02 90 / 0.12)` | Dialog / Drawer (관리자 화면, Phase 7) |
...
- 배경 `--color-surface`, 경계 1px `--color-border`, 그림자 없음(`elev-0`)
- **카드 안에 카드를 넣지 않습니다.** 내부 구획이 필요하면 구분선(`--color-border`) 또는 여백으로 나눕니다
- 카드가 클릭 가능하면 전체를 `<a>`/`<button>`으로 감싸고 hover 시 경계선만 `--color-border-strong`으로 바꿉니다
- 상태 배지는 카드 우상단, `radius-xs`, `caption` 크기
...
```
┌────────────────────────────────────────────┐
│  Header — 로고(텍스트) + 아주 얇은 하단선  │  높이 56px(모바일)/64px
├────────────────────────────────────────────┤
│  Hero                                       │  상하 여백 64/96px
│    h1: 나를 닮은 교직 스타일을 찾아보세요  │
│    body-lg muted: 40문항 · 약 10분 · 가입 없음
├────────────────────────────────────────────┤
│  닉네임 입력 (인라인)                       │
├────────────────────────────────────────────┤
│  활성 검사 — 큰 카드 1개                    │  radius-lg, elev-1
│    검사명 / 설명 / 소요시간 / 문항수         │
│    [자세히 보기 ▾ Accordion]                │
│    [검사 시작하기] (primary lg)             │
│    (진행 중이면 [이어서 하기]가 primary)    │
├────────────────────────────────────────────┤
│  준비 중인 검사 — 작은 카드 3개 그리드      │  radius-md, 상태 배지, 비활성
├────────────────────────────────────────────┤
│  이용 안내 / FAQ — Accordion                │
├────────────────────────────────────────────┤
│  Footer — 저장 데이터 삭제 링크 + 안내      │
└────────────────────────────────────────────┘
```

**위계 규칙**

- 활성 검사 카드는 준비 중 카드보다 **면적 기준 3배 이상** 크게
- 준비 중 카드는 `--color-surface-muted` 배경 + `--color-foreground-muted` 글자 + "준비 중" 배지
- 준비 중 카드는 클릭 불가 (`aria-disabled`, 커서 default)
- 페이지 최대 폭 **72rem(1152px)**, 좌우 패딩 20/24px
- **MVP가 비어 보이지 않도록** 준비 중 검사 3개 + FAQ 4~6개를 반드시 채웁니다
...
| # | 금지 | 대신 |
|---|---|---|
| 1 | AI 서비스 같은 보라/파랑 gradient | 단색 세이지 + 웜 뉴트럴 |
| 2 | sparkle(✨) 아이콘 남발 | 아이콘 없이 텍스트, 또는 의미 있는 아이콘 1개 |
| 3 | 과도한 glassmorphism (반투명 블러 카드) | 불투명 surface + border |
| 4 | floating blob / 배경 물방울 도형 | 여백 |
| 5 | 모든 요소를 pill(`radius-full`)로 | `radius-md` 기본, pill은 라디오·진행바만 |
| 6 | 지나치게 둥근 카드(24px+) | 최대 18px |
| 7 | 카드 안 카드 안 카드 | 구분선 또는 여백으로 구획 |
| 8 | 과도한 emoji | 사용하지 않음. 필요하면 아이콘 |
| 9 | 유아적인 학교 캐릭터 / 일러스트 | 타이포그래피와 여백 |
| 10 | 의미 없는 decoration (점선 테두리, 리본 등) | 제거 |
| 11 | 그림자로 위계 만들기 | 크기 → 여백 → 굵기 → 색 순서 |
...
| 17 | 좌우 슬라이드 페이지 전환 | opacity fade |
| 18 | 4글자 코드·MBTI 명칭 노출 | 자체 결과 제목만 |
| 19 | "궁합", 등급, 순위 표현 | "호흡이 자연스러운 / 조율하면 더 편한" |
| 20 | 토큰에 없는 색·간격 임의 사용 | 먼저 이 문서에 토큰 추가 |
